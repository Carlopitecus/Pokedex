...
export default {
  ...
  methods: {
    async searchPokemon() {
      try {
        const pokemonIDLower = this.pokemonID.toLowerCase();
        const pokemonToFind = await fetch(`${pokeapi}/${pokemonIDLower}`)
...
<template>
  <div>
    ...
    <header class="header">
      <label for="">
        Typy pokemon Name or ID
        <input type="text" 
        v-model="pokemonID" v-on:keyup.enter="searchPokemon">
        <button
         class="searchButton"
        @click="searchPokemon" > Buscar Pokemon</button>
      </label>
    </header>

    <main
     class="main" v-if="Object.entries(pokemonData).length > 0">
      <seccion class="pokemonCard">
      <div class="nameImage">
